...
public class ObjectTracker {
  ...
  private RectF downscaleRect(final RectF fullFrameRect) {//缩小
    return new RectF(
  ...
  private RectF upscaleRect(final RectF downsampledFrameRect) {//放大
    return new RectF(
   ...
   */
  public class TrackedObject { //一个被跟踪过的目标
    private final String id;//id

    private long lastExternalPositionTime;//上次位置的时间

    private RectF lastTrackedPosition;//上次位置
    private boolean visibleInLastFrame;//在上一帧中是否可见

    private boolean isDead;//是否遗弃

    TrackedObject(final RectF position, final long timestamp, final byte[] data) {
      isDead = false;

      id = Integer.toString(this.hashCode());//id = hashcode

      lastExternalPositionTime = timestamp;//时间戳

      synchronized (ObjectTracker.this) {
        registerInitialAppearance(position, data);//注册对象
        setPreviousPosition(position, timestamp);//设置上次位置（更新）
        trackedObjects.put(id, this); //跟踪list中加入
      }
    }

    public void stopTracking() {//停止跟踪
      checkValidObject();
    ...
    public float getCurrentCorrelation() {
      checkValidObject();
      return ObjectTracker.this.getCurrentCorrelation(id);//id关联关系
    }

    void registerInitialAppearance(final RectF position, final byte[] data) {//注册对象？
      final RectF externalPosition = downscaleRect(position);//4个值分别/2
      registerNewObjectWithAppearanceNative(id, //注册新的对象
    ...
    synchronized void setPreviousPosition(final RectF position, final long timestamp) {
      checkValidObject();
      synchronized (ObjectTracker.this) {
        if (lastExternalPositionTime > timestamp) {//检查时间
        ...
        updateTrackedPosition();//更新位置
      }
    }

    void setCurrentPosition(final RectF position) {//设置当前位置
      checkValidObject();
    ...
    private synchronized void updateTrackedPosition() {
      ...
      lastTrackedPosition = new RectF(delta[0], delta[1], delta[2], delta[3]);//更新位置

      visibleInLastFrame = isObjectVisible(id);//是否上一帧可见
    }

    public synchronized RectF getTrackedPositionInPreviewFrame() {//在上一帧中得到跟踪位置
      checkValidObject();

      if (lastTrackedPosition == null) {//无上次位置
        return null;
      }
      return upscaleRect(lastTrackedPosition);//放大
    }
    ...
    private void checkValidObject() {//检测对象是否有效
      if (isDead) {
        throw new RuntimeException("TrackedObject already removed from tracking!");
      } else if (ObjectTracker.this != instance) {//属于另一个对象
